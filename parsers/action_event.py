...
class ActionEventParser(BaseEventParser):
    async def parse(self, event: dict, match_id: int) -> dict | None:
        self.log("Parsing action event for match_id: %d", match_id, level=20)  # INFO
        self.log("Event data: %s", event, level=10)  # DEBUG

        try:
            parsed_event = await asyncio.to_thread(self._parse_event, event, match_id)
            self.log("Successfully parsed action event for match_id: %d", match_id, level=20)  # INFO
            self.log("Parsed data: %s", parsed_event, level=10)  # DEBUG
            return parsed_event
        except Exception:
            self.logger.exception("Error while parsing action event for match_id: %d", match_id)
            return None
